...
import { calculateSkip, createPaginationResult, createSortObject, toPaginatedListResponse } from './paginator';
...
describe('paginator utilities', () => {
  ...
  it('wraps list response meta for Respond helper', () => {
    const result = createPaginationResult([{ id: 1 }], 1, 1, 10);
    expect(toPaginatedListResponse(result).meta.pagination.total).toBe(1);
  });
});

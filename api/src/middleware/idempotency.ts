...
/**
 * Idempotency middleware for unsafe HTTP methods.
 *
 * - Header: `Idempotency-Key` (see IDEMPOTENCY_KEY_HEADER)
 * - TTL: `IDEMPOTENCY_TTL_SECONDS` env (default 86400s / 24h), stored in Redis
 * - Same key + different payload → 409 IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_PAYLOAD
 * - Same key + in-flight request → 409 IDEMPOTENT_REQUEST_IN_PROGRESS
 * - Same key + identical payload → replays cached JSON response (Idempotency-Status: replayed)
 * - Redis unavailable → middleware is a no-op (request proceeds)
 */
